...
## Backlog → presunutý do `docs/backlog.md`

> **Živý zoznam otvorených úloh už nie je tu.** Býval uprostred tohto súboru,
> kde sa zle hľadal (v Obsidiane bolo treba scrollovať cez stovky riadkov
> histórie). Od 19.8.2026 žije samostatne v **`docs/backlog.md`** — na jeden
> `Cmd + O`.
>
> **Deľba práce medzi súbormi:** denník = **história** (čo sa kedy stalo, prečo
> sme sa tak rozhodli, ponaučenia), backlog = **prítomnosť** (čo je otvorené a
> v akom poradí). SessionStart hook (`.claude/hooks/session-start.sh`) číta
> backlog priamo z nového súboru.
